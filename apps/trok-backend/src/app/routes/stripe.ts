...
router.post(
	...
	async (req: express.Request, res: express.Response, next: NextFunction) => {
		...
		let obj, data, transaction, event: Stripe.Event = req.body;
		try {
		...
		switch (event.type) {
			case 'issuing_authorization.created':
				...
				console.log('************************************************');
				console.log('NEW AUTHORIZATION:', transaction);
				console.log('************************************************');
				break;
...
router.post(
	...
	async (req: express.Request, res: express.Response, next: NextFunction) => {
		...
		try {
			event = stripe.webhooks.constructEvent(req.body, <string>sig, authWebhookSecret);
			console.log('✅  Success Webhook verified!');
		} catch (err) {
			// On error, log and return the error message
			if (err instanceof Error) {
		...
		switch (event.type) {
			case 'issuing_authorization.request':
				...
				const status = await handleAuthorizationRequest(data);
				console.log('-----------------------------------------------');
